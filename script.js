...
function createTask(id, task, color) {
  ...
  taskHeader.addEventListener("click", function () {
    // class -> change
    // get all the classes on element
    ...
    let nextIdx = (idx + 1) % 4; // to keep it in range
...
for (let i = 0; i < colorBtns.length; i++) {
    colorBtns[i].addEventListener("click",function(){
        let filteredColorCard = colorBtns[i].classList[1];
        console.log(filteredColorCard);
        filterCards(filteredColorCard);
